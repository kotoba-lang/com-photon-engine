(ns photon-engine.main
  "Kotodama WASM entrypoint for the Photon_engine clean-room actor (L4) — Clojure port.
  ...
  facts (`photon_engine.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "photon_engine")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"photon_engine.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "photon_engine-compat" :tier tier :entities entities} 200])
...
(defn start! [] :photon_engine-compat/ready)
